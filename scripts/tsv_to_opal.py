...
def process_bracken_files_to_opal(input_dir, output_dir):
    ...
    for file_name in os.listdir(input_dir):
        if file_name.endswith(".bracken.report.txt"):
            sample_name = file_name.split(".")[0]  # Extract sample name
    ...
    for sample_name, files in sample_files.items():
        ...
        for file_name in files:
            # Determine rank based on the file name
            rank = "unknown"
            ...
            # Process the file and create OPAL-compatible columns
        ...
        # Concatenate all data for the sample
        if combined_data:
            final_data = pd.concat(combined_data)

            # Add the OPAL-required header
            header = (
...
if __name__ == "__main__":
    # Example usage
    input_directory = "."  # Directory containing Bracken files
    output_directory = "./opal_formatted_files"  # Directory to save formatted files
